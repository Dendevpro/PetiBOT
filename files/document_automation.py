...
try:
    ...
    print("Please run: pip install python-docx Pillow PyPDF2 reportlab openai")
...
class DocumentAutomation:
    ...
    def summarize_with_gpt(self, text: str) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "Você é um assistente especializado em resumir documentos jurídicos em português brasileiro de forma concisa."},
                    {"role": "user", "content": f"Resuma o seguinte documento jurídico em no máximo 500 caracteres, destacando os pontos principais:\n\n{text}"}
                ],
                max_tokens=300,
                temperature=0.3
            )
            ...
            # Ensure summary fits in QR code
            if len(summary) > 500:
                summary = summary[:497] + "..."
            
            return summary
        except Exception as e:
            raise Exception(f"Error generating summary with GPT: {e}")
    ...
    def add_qr_to_pdf_header(self, pdf_path: str, qr_image_path: str, output_path: str) -> str:
        """
        Add QR code to PDF header on first page only
        ...
        try:
            ...
            # Position QR code - smaller and tighter to corner
            page_width, page_height = letter
            qr_size = 50  # Reduced to 50x50 pixels
            margin_right = 25  # 25 pixels from right edge
            margin_top = 25    # 25 pixels from top edge
            
            # Calculate position (top-right corner)
            qr_x = page_width - margin_right - qr_size
            qr_y = page_height - margin_top - qr_size
            
            # Add "Resumo" text above QR code - centered
            can.setFont("Helvetica-Bold", 8)  # Smaller font for smaller QR
            
            # Calculate center position for text
            text_width = can.stringWidth("Resumo", "Helvetica-Bold", 8)
            text_x = qr_x + (qr_size / 2) - (text_width / 2)  # Center text above QR
            text_y = qr_y + qr_size + 4  # 4 pixels above QR
            can.drawString(text_x, text_y, "Resumo")
            ...
            for page_num in range(len(reader.pages)):
                ...
                # Only merge overlay on first page
                if page_num == 0:
