...
class DocumentAutomation:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.gemini_api_key:
            ...
            if not self.model:
                # If none work, just use the first one and let it fail with a proper error
    ...
    def summarize_with_gemini(self, text: str) -> str:
        ...
        """
        try:
            # First, let's try to list available models to help debug
            try:
            ...
            if "404" in error_msg or "not found" in error_msg:
                raise Exception(
                    ...
                    "Possíveis soluções:\n"
                    "1. Verifique se sua API key é válida em: https://makersuite.google.com/app/apikey\n"
                    "2. Crie uma NOVA API key (às vezes keys antigas param de funcionar)\n"
                    "3. Verifique se a API do Gemini está habilitada para sua conta"
                )
