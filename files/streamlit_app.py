...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("🔑 API Keys", expanded=not st.session_state.config.get('claude_api_key')):
            claude_key = st.text_input(
                "Claude API Key",
                value=st.session_state.config.get('claude_api_key', ''),
                type="password",
                help="Obtenha sua chave em: https://console.anthropic.com/settings/keys"
            )
            
            if claude_key != st.session_state.config.get('claude_api_key', ''):
                st.session_state.config['claude_api_key'] = claude_key
        ...
        config_status = {
            "Claude API": bool(st.session_state.config.get('claude_api_key')),
    ...
    with tab1:
        ...
        if not st.session_state.config.get('claude_api_key'):
            st.markdown('<div class="error-box">⚠️ Configure a API Key do Claude na barra lateral para começar.</div>', unsafe_allow_html=True)
            return
    ...
    with tab3:
        ...
        2. **🤖 Resumo com IA**: Gera um resumo conciso usando Claude AI da Anthropic
        ...
        - **Python 3.8+**
        - **LibreOffice** (para conversão Word → PDF local) ou **CloudConvert** (nuvem)
        - **Claude API Key** (paga, ~$0.01 por documento)
        - **Email configurado** (opcional, para envio automático)
        
        ### 📚 Tecnologias
        
        - **Streamlit**: Interface web
        - **Claude AI (Anthropic)**: Geração de resumos com IA
        ...
        - Certifique-se de que o LibreOffice está instalado (local) ou CloudConvert configurado (nuvem)
        - Confira os logs de erro no histórico
        
        ---
        
        **Versão 2.0** | Desenvolvido para automação de documentos jurídicos
        """)
...
def process_document(docx_path: Path, recipient_email: str, send_email: bool):
    ...
    try:
        ...
        status_text.text("🤖 Gerando resumo com Claude AI...")
        progress_bar.progress(40)
        summary = automation.summarize_with_claude(text)
        ...
        with st.expander("🔍 Dicas de Solução"):
            ...
            **Erros comuns e soluções:**
            ...
            - **Erro de conversão PDF**: Instale o LibreOffice ou configure CloudConvert API
            - **Erro Claude API**: Verifique sua API key em https://console.anthropic.com/settings/keys
