...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("🔑 API Keys", expanded=not st.session_state.config.get('openai_api_key')):
            openai_key = st.text_input(
                "OpenAI API Key",
                value=st.session_state.config.get('openai_api_key', ''),
                type="password",
                help="Obtenha sua chave em: https://platform.openai.com/api-keys ($5 grátis para novas contas)"
            )
            
            if openai_key != st.session_state.config.get('openai_api_key', ''):
        ...
        config_status = {
            "OpenAI API": bool(st.session_state.config.get('openai_api_key')),
    ...
    with tab1:
        ...
        if not st.session_state.config.get('openai_api_key'):
            st.markdown('<div class="error-box">⚠️ Configure a API Key do OpenAI na barra lateral para começar.</div>', unsafe_allow_html=True)
            return
    ...
    with tab3:
        ...
        2. **🤖 Resumo com IA**: Gera um resumo conciso usando OpenAI GPT-4o-mini
        ...
        - **OpenAI API Key** ($5 grátis para novas contas, ~$0.002 por documento)
        - **Email configurado** (opcional, para envio automático)
        
        ### 📚 Tecnologias
        
        - **Streamlit**: Interface web
        - **OpenAI GPT-4o-mini**: Geração de resumos com IA
...
def process_document(docx_path: Path, recipient_email: str, send_email: bool):
    ...
    try:
        ...
        status_text.text("🤖 Gerando resumo com GPT-4o-mini...")
        progress_bar.progress(40)
        summary = automation.summarize_with_gpt(text)
        ...
        with st.expander("🔍 Dicas de Solução"):
            ...
            **Erros comuns e soluções:**
            ...
            - **Erro OpenAI API**: Verifique sua API key em https://platform.openai.com/api-keys
